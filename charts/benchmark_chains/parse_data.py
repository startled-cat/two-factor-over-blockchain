...
def add_network_data_to_db(network_data, db_path):
    ...
    for (network, data) in network_data:
        for m in data:
            c.execute('''INSERT INTO benchmark_data VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''', (network, m["datetime"], m["user_time"][0], m["user_time"][1], m["user_time"][2], m["user_time"][3], m["user_time"][4], m["user_gas_used"], m["user_gas_price"], m["user_cost"], m["app_time"][0], m["app_time"][1], m["app_time"][2], m["app_time"][3], m["app_time"][4], m["app_gas_used"], m["app_gas_price"], m["app_cost"]))
    conn.commit()
    conn.close()
